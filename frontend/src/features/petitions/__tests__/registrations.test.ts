...
describe('petitionsRowActions', () => {
  it('registers View/Edit/Delete/Print/Assign in order', () => {
 ...
 */
describe('petitionsRowActions — in chứng từ từ danh sách', () => {
  it('có hành động in, nằm NGAY TRÊN cột Thao tác chứ không nấp trong menu', () => {
    const inChungTu = petitionsRowActions.all().find((a) => a.key === 'print');
  ...
  it('bấm in mở modal với ĐÚNG thực thể và id của dòng', () => {
    ...
    inChungTu.execute({ id: 'P9', status: 'MOI_TIEP_NHAN' }, ctx);
