...
describe('PetitionListPageShell — bố cục theo hệ cũ', () => {
  ...
  it('cột hệ cũ KHÔNG có thì ẩn sẵn, bật lại được từ menu chọn cột', async () => {
    ...
    for (const an of ['Đối tượng bị tố', 'Hạn xử lý', 'Ngày tạo']) {
...
describe('PetitionListPageShell — ô tìm kiếm dạng thẻ', () => {
   ...
   * Đơn thư ẩn sẵn ba cột. Mệnh đề còn nghĩa và được giữ: cột đang HIỆN xếp TRƯỚC, cột ẩn nằm
   * trong nhóm "Cột khác".
   */
  it('gợi ý phủ cả cột ẩn, cột đang HIỆN xếp trước', async () => {
    ...
    const iAn = goiY.findIndex((t) => t.includes('Đối tượng bị tố'));
