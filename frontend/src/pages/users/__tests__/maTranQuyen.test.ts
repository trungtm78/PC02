...
describe('dungMaTran', () => {
  it('hàng = mọi subject của danh mục, cột = mọi action của danh mục (kể cả action lạ ngoài bộ nhãn)', () => {
    ...
    expect(mt.actions).toEqual(['read', 'write', 'restore', 'review_reset_request']);
  });
  ...
  it('quyền vai trò đang giữ mà danh mục không có → vẫn giữ nguyên khi lưu (không âm thầm bỏ)', () => {
    const mt = dungMaTran(DANH_MUC, [
      { action: 'read', subject: 'Case' },
      { action: 'fly', subject: 'Case' },
    ]);
    expect(danhSachGui(mt)).toEqual(
      expect.arrayContaining([{ action: 'fly', subject: 'Case' }]),
    );
  });
});
