...
describe('DelegationsController — delegation', () => {
  let controller: DelegationsController;

  beforeEach(async () => {
    const module = await buildControllerModule(DelegationsController, DelegationsService, mockService);
    controller = module.get(DelegationsController);
    jest.clearAllMocks();
  });
  ...
  it('create() delegates to service.create with dto, userId and audit info', async () => {
    mockService.create.mockResolvedValue({ data: { id: 'd1' } });
    const req = makeReq();
  ...
  it('delete() delegates to service.delete with id, userId, audit, dataScope', async () => {
    ...
    const req = makeReq();
    await controller.delete('del-1', mockUser, req);
