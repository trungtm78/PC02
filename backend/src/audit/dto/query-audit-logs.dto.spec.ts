...
/**
 * Tầng ValidationPipe của thẻ tìm kiếm Nhật ký hoạt động — kiểm bằng đúng cấu hình pipe toàn cục
...
async function kiem(giaTri: Record<string, unknown>) {
  const dto = plainToInstance(QueryAuditLogsDto, giaTri);
...
describe('QueryAuditLogsDto — thẻ tìm kiếm', () => {
  it('một thẻ (`?tk=x`) → mảng một phần tử', async () => {
    const { dto, errors } = await kiem({ tk: 'nguoiThucHien~nguyen' });
    expect(dto.tk).toEqual(['nguoiThucHien~nguyen']);
  ...
  it('`search` cũ vẫn nhận', async () => {
    expect((await kiem({ search: 'admin' })).errors).toEqual([]);
  });
});
