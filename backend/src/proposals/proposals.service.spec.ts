...
describe('ProposalsService — create()', () => {
  ...
  it('uses provided proposalNumber and skips commitWithTx', async () => {
    ...
    const result = await service.create(
      { proposalNumber: 'DX-MANUAL-001', content: 'test' } as any,
      'u1',
    );
  ...
  it('rolls back counter when proposal.create throws inside transaction', async () => {
    ...
    await expect(
      service.create({ content: 'test' } as any, 'u1'),
  ...
  describe('case-linked proposal', () => {
    ...
    it('throws ForbiddenException when relatedCase is out of scope', async () => {
      mockPrisma.proposal.findFirst.mockResolvedValue({
        ...FAKE_PROPOSAL_WITH_CASE,
        relatedCase: {
          ...FAKE_PROPOSAL_WITH_CASE.relatedCase,
 ...
 * and no scope check. A prosecutor proposal could be filed against any case in the system.
 * It is optional, so the check applies only when a case is actually named.
 */
describe('ProposalsService — create scope', () => {
  let service: ProposalsService;
