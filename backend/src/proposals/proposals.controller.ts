...
export class ProposalsController {
  ...
  getList(@Query() query: QueryProposalsDto) {
    return this.proposalsService.getList(query);
  }
  ...
  create(@Body() dto: CreateProposalDto, @CurrentUser() user: AuthUser, @Req() req: Request) {
  ...
  update(@Param('id') id: string, @Body() dto: Partial<CreateProposalDto>, @CurrentUser() user: AuthUser, @Req() req: Request) {
    return this.proposalsService.update(id, dto, user.id, { ipAddress: req.ip, userAgent: req.headers['user-agent'] });
  }
  ...
  delete(@Param('id') id: string, @CurrentUser() user: AuthUser, @Req() req: Request) {
    return this.proposalsService.delete(id, user.id, { ipAddress: req.ip, userAgent: req.headers['user-agent'] });
  }
}
