...
describe('LawyersController — delegation', () => {
  let controller: LawyersController;

  beforeEach(async () => {
    const module = await buildControllerModule(LawyersController, LawyersService, mockService);
    controller = module.get(LawyersController);
    jest.clearAllMocks();
  });
  ...
  it('delete() delegates to service.delete with id, userId, audit, dataScope', async () => {
    ...
    const req = makeReq();
    await controller.delete('law-1', mockUser, req);
