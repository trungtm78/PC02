...
const mockPrisma = { investigationSupplement: { findMany: jest.fn(), findFirst: jest.fn(), count: jest.fn(), create: jest.fn() } };
const mockAudit = { log: jest.fn() };

const FAKE_SUPPLEMENT = {
  id: 'is-001', caseId: 'case-001', type: 'TYPE_A', deletedAt: null,
...
describe('InvestigationSupplementsService — scope enforcement', () => {
  ...
  it('returns supplement when found (no scope)', async () => {
    mockPrisma.investigationSupplement.findFirst.mockResolvedValue(FAKE_SUPPLEMENT);
    const result = await service.getById('is-001');
    expect(result.success).toBe(true);
  });

  it('throws NotFoundException when not found', async () => {
    mockPrisma.investigationSupplement.findFirst.mockResolvedValue(null);
  ...
  it('throws ForbiddenException when parent case is out of scope', async () => {
    mockPrisma.investigationSupplement.findFirst.mockResolvedValue({ ...FAKE_SUPPLEMENT, case: { ...FAKE_SUPPLEMENT.case, assignedTeamId: 'team-X', investigatorId: 'user-X' } });
  ...
  it('passes with null scope (admin bypass)', async () => {
    mockPrisma.investigationSupplement.findFirst.mockResolvedValue({ ...FAKE_SUPPLEMENT, case: { ...FAKE_SUPPLEMENT.case, assignedTeamId: 'team-X' } });
