...
describe('PetitionsService — một nguồn điều kiện lọc cho danh sách, thẻ số, xuất', () => {
  ...
  it('điều kiện thẻ số = điều kiện danh sách, chỉ khác phần trạng thái', async () => {
    ...
    const trangThai = danhSach.status as { equals: unknown; notIn: unknown };
    expect(trangThai.equals).toBe(PetitionStatus.DANG_XU_LY);
    expect(Array.isArray(trangThai.notIn)).toBe(true);
    expect(Object.keys(trangThai).sort()).toEqual(['equals', 'notIn']);
