 ...
 * Nguồn: provinces.open-api.vn — 10,051 đơn vị hành chính cấp xã
 *
 * Run: npx ts-node prisma/seed-wards.ts
 * Idempotent: uses upsert on type+code unique constraint.
 * TPHCM ưu tiên (order=0), tỉnh khác (order=1).
 *
 * Sau khi seed, API /directories?type=WARD sẽ trả về đầy đủ phường/xã toàn quốc.
 * Dữ liệu đầy đủ tại: frontend/src/data/wards-full.json (10,051 đơn vị)
 */
...
// ─── Kết hợp tất cả — TPHCM ưu tiên đầu (fallback khi không có JSON) ────────
const FALLBACK_WARDS: WardEntry[] = [
  ...HCM_WARDS,
  ...HN_WARDS,
  ...DN_WARDS,
  ...HP_WARDS,
...
async function loadWards(): Promise<WardEntry[]> {
  // Try loading full data from JSON (generated by scripts/fetch-wards.mjs + split-wards.mjs)
  const jsonPath = require('path').join(__dirname, '../../frontend/src/data/wards-full.json');
  try {
    const data = require(jsonPath);
    console.log(`  Loaded ${data.length} wards from wards-full.json`);
    return data as WardEntry[];
  } catch {
    console.log(`  wards-full.json not found — using fallback ${FALLBACK_WARDS.length} wards`);
    console.log(`  Tip: Run 'node scripts/fetch-wards.mjs && node scripts/split-wards.mjs' to generate full data`);
...
export async function seedWards(prismaClient: PrismaClient): Promise<void> {
  ...
  const BATCH = 200;
  for (let i = 0; i < ALL_WARDS.length; i += BATCH) {
    const batch = ALL_WARDS.slice(i, i + BATCH);
