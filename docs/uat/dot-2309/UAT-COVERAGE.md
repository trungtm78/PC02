...
**136 ca / 10 nhóm A–J.** Số 136 là `TC_min` do `coverage_calc.py` tính từ `_coverage-ledger.md`, KHÔNG phải con số đặt ra: `M1=136 · M2=120 · M3=33 · M4=120 → MAX=136`. Độ phủ 130/136 item (95,6%) — 6 item còn lại là GAP có lý do, khai rõ trong ledger.
...
| ID | Mệnh đề | Luật | Loại | Ưu tiên | Chạy | Kết quả |
|---|---|---|---|---|---|---|
...
| A4 | Gập KHÔNG phải lý do loại cột | R1-HIDDEN | RED | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
...
| A15 | Mọi cột trong tệp mang giá trị, không phải ô trống | R1-VALUE | GREEN | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| A16 | Khoá lưu đọc đúng cho cột riêng | R1-VALUE | GREEN | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| A17 | Khoá lưu đọc đúng cho khoá metadata | R1-VALUE | GREEN | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| A18 | Cột đã cắt mà có dữ liệu thì phép đo báo ĐỎ | R1-GUARD | RED | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| A19 | Phép đo trong deploy chỉ cảnh báo, không chặn deploy | R1-GUARD | EDGE | P2 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| A20 | Xuất đúng trần 5.000 dòng | R1-CAP | BOUNDARY | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| A21 | Quá trần thì cắt còn 5.000 và nói rõ đã cắt | R1-CAP | BOUNDARY | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| A22 | Thiếu quyền xuất đầy đủ thì bị từ chối | R1-ALL | SECURITY | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| A23 | Vai vô danh không gọi được đường xuất | R1-ALL | SECURITY | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| A24 | Kết cặp nguồn đơn x loại thông tin x trạng thái trên tệp xuất | R1-VALUE | EP | P2 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| A25 | Vượt trần xuất thì báo trước, không để tải xong mới hỏng | R1-CAP | UX | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
...
| B8 | Bấm chuột trên nút không lan ra dòng | R2-STOP | STATE | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B9 | Gõ Enter trên nút không lan ra dòng | R2-STOP | STATE | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B10 | Biểu tượng là bút, cùng biểu tượng sửa của hệ | R2-ICON | GREEN | P2 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B11 | Vùng chạm của nút đạt tối thiểu WCAG 2.2 | R2-ICON | A11Y | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B12 | Tải tệp lên từ ngay trong danh sách | R2-FILE | GREEN | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B13 | Tải tệp xuống từ ngay trong danh sách | R2-FILE | GREEN | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B14 | Tên tệp tải về theo máy chủ, không bị ép tên | R2-FILE | RED | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B15 | Nhãn đọc được nói rõ sửa ô nào của hồ sơ nào | R2-A11Y | A11Y | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B16 | Tới được nút bằng phím Tab | R2-A11Y | A11Y | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B17 | Lưu xong bảng cập nhật ngay, không cần tải lại | R2-SAVE | GREEN | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B18 | Lưu hỏng thì báo lỗi và KHÔNG mất chữ đã gõ | R2-SAVE | RED | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B19 | Sửa nhanh hồ sơ ngoài phạm vi bị chặn | R2-SAVE | SECURITY | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B20 | Tệp ngoài danh sách loại cho phép bị từ chối | R2-FILE | SECURITY | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B21 | Tên tệp chứa ký tự đường dẫn bị vô hiệu hoá | R2-FILE | SECURITY | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B22 | Đóng popup rồi mở lại thì lấy giá trị từ máy chủ | R2-SAVE | STATE | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B23 | Lưu xong rồi mở dòng KHÁC không mang giá trị dòng trước | R2-SAVE | STATE | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B24 | Hai tab cùng mở, tab A sửa nhanh, tab B tải lại thấy đúng | R2-SAVE | STATE | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B25 | Sửa nhanh: ô rỗng, có quyền sửa | R2-FILLED | DECISION | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B26 | Sửa nhanh: ô có chữ, có quyền sửa | R2-FILLED | DECISION | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B27 | Sửa nhanh: ô rỗng, không quyền sửa | R2-FILLED | DECISION | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B28 | Sửa nhanh: ô có chữ, không quyền sửa | R2-FILLED | DECISION | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B29 | Popup sửa nhanh có lối thoát rõ ràng | R2-SAVE | UX | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B30 | Nút sửa nhanh nhất quán với chỗ sửa khác trong hệ | R2-ICON | UX | P2 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B31 | Thông báo lỗi nói được cách khắc phục | R2-SAVE | UX | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B32 | Sửa từ bảng ít thao tác hơn mở hồ sơ | R2-EMPTY | UX | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B33 | Popup có đủ trạng thái tải, rỗng, lỗi, khoá | R2-SAVE | UX | P2 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B34 | Tiêu điểm không bị che khi cuộn bảng | R2-A11Y | A11Y | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B35 | Tương phản chữ trên nút đạt mức AA | R2-ICON | A11Y | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B36 | Mọi nút có tên đọc được | R2-A11Y | A11Y | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B37 | Popup giữ tiêu điểm đúng cách và Esc thoát được | R2-SAVE | A11Y | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| B38 | Quét tự động không còn lỗi mức A/AA | R2-A11Y | A11Y | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
...
| C4 | Danh mục rỗng thì báo hỏng, không im lặng | R3-DATA | RED | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| C5 | Cột Loại thông tin có trong tệp xuất | R3-EXPORT | GREEN | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
| C6 | Trạng thái rỗng của bảng phân biệt với tải hỏng | R3-DATA | UX | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
...
| E7 | Chạy seed lại lần nữa vẫn an toàn | R5-SEED | DATA | P1 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
...
| F6 | Gieo lỗi: bỏ ô khỏi thân lời gọi thì cổng phải ĐỎ | R6-KEEP | RED | P0 |  | **CHƯA CHẠY** — chặn ở tài khoản thử |
...
| J9 | Máy chủ báo mã bản dựng khớp commit đã hợp nhất | X5 | GREEN | P0 | x | **ĐẠT** — đo 23/09 04:38, buildId `290f65d17c` khớp `origin/main` |
...
| J12 | Sau deploy, đường kiểm tra sức khoẻ trả mã bản dựng đúng | X5 | GREEN | P0 | x | **ĐẠT** — đo 23/09 04:38, buildId `290f65d17c` khớp `origin/main` |

---

## Vì sao chưa dòng nào PASS
...
Ba ca phạm vi dữ liệu (`COV-R7-SCOPE-2`, `COV-X6-1`, `COV-X6-2`) cần **thêm** một tài khoản cán bộ thuộc tổ khác — không có tài khoản thứ hai thì không chứng minh được ranh giới phạm vi, chỉ chứng minh được là mình thấy hồ sơ của mình.

Em KHÔNG ghi PASS cho ca nào chưa chạy, và KHÔNG suy PASS từ ca kiểm đơn vị: mệnh đề nói về sản phẩm thì bằng chứng phải đi qua giao diện hoặc HTTP.

## Trần kết luận hiện tại

`self_audit.py` → **SẴN SÀNG-KÈM-KHUYẾT**. Ba phép đo máy không làm được vẫn trống: gieo lỗi, soát đối kháng, đánh giá trải nghiệm bằng người đóng vai thật.
